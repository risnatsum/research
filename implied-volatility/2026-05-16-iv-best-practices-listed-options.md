...
1. **Forward extraction at the ATM-forward strike is the single most important step.** CBOE's VIX methodology picks the strike where `|C − P|` is smallest, then inverts put-call parity at that strike to recover `F`. This works because vega is highest near ATM (quotes are tightest) and any multiplicative vol mis-mark cancels between the call and put legs (CBOE 2024).
   ...
   *Plain English:* The "forward price" is what the market thinks a stock will be worth at expiry, after dividends and financing costs. Rather than estimating this yourself, you can read it straight from the options market. The trick: find the strike where the call and put cost almost the same. At that point the tiny difference in price tells you exactly what the market's forward price is — no guessing required. AAPL example: if AAPL is at $200 and the $195 call and $195 put are only $0.10 apart, the market is saying the forward price is roughly $195.10. Use that number, not your own dividend estimate.
...
2. **Implied dividends beat consensus dividend forecasts — and the gap is tradeable.** OptionMetrics now markets a dedicated implied-dividend product on the explicit grounds that consensus forecasts "can be prone to systematic bias" (OptionMetrics 2024). Eurex lists liquid single-name dividend futures on 80+ European names (Eurex factsheet). If you use IBES or Bloomberg BDVD for dividends in your IV inversion, you are imposing a biased view that shows up as fake skew.
   ...
   *Plain English:* To price an option you need to know what dividends the stock pays before expiry — a dividend drops the stock price, which affects whether your option finishes in the money. Analysts (IBES, Bloomberg) publish forecasts. But the options market has its own view baked into prices, and that view often differs. If you use the analyst number instead of the market-implied one, your error shows up as fake skew — it looks like puts are structurally more expensive than calls, when really it's just your dividend input being off.
...
3. **The forward-based ("Buehler") framework eliminates the ex-div kink in the IV surface.** Escrowed-spot models attach volatility to a contaminated state variable, so IV jumps discontinuously the day a dividend rolls off the PV stack. Buehler (2010) showed defining vol on the dividend-stripped forward process removes this artefact entirely. This is now standard on single-name equity derivatives desks.
   ...
   *Plain English:* On the day a stock goes ex-dividend, the stock price drops by roughly the dividend amount. Older models produce a sudden jump in implied volatility on that day — even though nothing about the company's actual risk changed. It's a maths artefact, not a real signal. Buehler's fix: attach volatility to the "clean" price with dividends stripped out, rather than the raw stock price. The IV surface stays smooth across the ex-div date, and you're measuring actual uncertainty rather than a dividend accounting glitch.
...
4. **Parametric arbitrage-free surfaces are not optional.** Gatheral & Jacquier's SSVI (2014) gives a closed-form surface that is provably free of butterfly and calendar arbitrage under explicit parameter conditions. Linear interpolation in raw IV does not — it routinely produces negative implied densities. Hagan's original 2002 SABR formula has the same problem at low strikes; "arbitrage-free SABR" (Hagan 2014) and Antonov's approximations are the rates-desk fixes.
   ...
   *Plain English:* You only have option prices at a handful of strikes. To price anything in between you need to fill in the gaps. Drawing a straight line between known IV points sounds harmless but breaks a fundamental rule: it can imply the stock has a negative probability of landing at certain prices — which is mathematically impossible and means someone could construct a guaranteed free profit. Professional desks use specific formula shapes (SSVI for indices, SABR for rates) that are designed so this never happens by construction. Example: naively interpolating IV between the $190 and $210 AAPL strikes might produce a slightly negative implied probability density around $200. SSVI cannot do this — the no-free-lunch condition is baked into the formula's parameters.
...
5. **Quote staleness is the silent killer.** Muravyev & Pearson (2020, *RFS*) showed option quotes lag underlying moves by ~30 seconds on average for liquid names, and conventional effective spreads overstate true costs by ~3x. If your IV pipeline does not timestamp-synchronise spot and option mid to the same second (or better, the same NBBO snapshot), your IVs are systematically wrong — and the error is invisible until you compare against a desk that does synchronise.
   ...
   *Plain English:* Option quotes don't update as fast as stock prices. If AAPL jumps from $200 to $203 in one second, the stock quote updates instantly — but the option market maker might still be showing a price from 30 seconds ago. If you combine the new $203 stock price with a stale option quote to compute IV, you get a wrong number. Not wrong by a little — wrong in a consistent direction that looks like real skew or term structure. The fix is to only pair a stock price and option quote that were recorded at the same moment. Most retail data pipelines don't do this. The scary part: the error is invisible because all your IVs are internally consistent — they're all just consistently wrong.
...
6. **Filter hierarchy that actually matters** (composite of CBOE, IvyDB, Goyal-Saretto): mid-quote only → bid > 0 → drop crossed/locked → drop two consecutive zero bids walking out from ATM → OTM only → delta in [0.05, 0.95] → drop T < 7 days → relative spread `(ask−bid)/mid` < 50% → no-arbitrage check (mid ≥ intrinsic). Volume and OI are *not* primary filters in either CBOE or IvyDB — they are exposed as fields for the user to apply.
   ...
   *Plain English:* Before inverting any option price to get IV, throw out the junk. What counts as junk: last-trade prices (use the midpoint of bid and ask — last trade could be hours old); options where nobody is bidding (the ask is meaningless with no buyer); options where bid is higher than ask (a data feed error); very deep ITM or OTM options (a $0.01 price error translates to a massive IV error when vega is near zero); anything expiring in less than a week (numbers get unstable near expiry); and options priced below their intrinsic value (e.g. a $200 call trading at $1 when AAPL is at $210 — that's a free $9, meaning the quote is broken). The counterintuitive one: high volume and open interest don't make the approved list. A busy option can still have a stale quote; a quiet one can have a perfectly valid midpoint. Filter on quote quality, not trading activity.
